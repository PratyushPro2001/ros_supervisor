...
# Serialize ROS access: rclpy is not thread-safe under concurrent FastAPI requests.
_API_ROS_LOCK = threading.Lock()


def _qos_profile_to_dict(q) -> dict:
    # Works with rclpy QoSProfile objects from endpoint info
    return {
...
def _qos_mismatch(pub_qos: dict, sub_qos: dict):
    ...
    if pub_qos.get("reliability") != sub_qos.get("reliability"):
        issues.append(
            f"reliability mismatch: pub={pub_qos.get('reliability')} sub={sub_qos.get('reliability')}"
        )
    if pub_qos.get("durability") != sub_qos.get("durability"):
        issues.append(
            f"durability mismatch: pub={pub_qos.get('durability')} sub={sub_qos.get('durability')}"
        )
    return issues
...
def api_qos_report():
    with _API_ROS_LOCK:
        ...
        # NOTE: if adapter.get_topic_names_and_types() returns dicts instead of tuples,
        # we'll fix that in the next step if needed.
...
def api_snapshot():
    """
    Fast endpoint: returns cached snapshot produced by background daemon.
    Does NOT touch ROS/rclpy.
    """
    from backend.api.snapshot_daemon import daemon
    return daemon.get_latest()
